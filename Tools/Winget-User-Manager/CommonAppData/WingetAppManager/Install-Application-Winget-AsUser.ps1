...
| (_| || | | || (_| || |   |  __/ \ V  V / \__ \| |_ | (_| || |_| || || (_) || |    _ | (__ | (_) || | | | | |
 \__,_||_| |_| \__,_||_|    \___|  \_/\_/  |___/ \__| \__,_| \__, ||_| \___/ |_|   (_) \___| \___/ |_| |_| |_|
                                                             |___/
.SYNOPSIS
  Installs an application using winget and adds scheduled task to update
.DESCRIPTION
.Installs Winget if missing
.Searches all Winget apps and displays GridView output
.Installs Application
.Installs Scheduled Task to update application
...
##Start Logging to %TEMP%\winget-installDATE.log
$date = get-date -format ddMMyyyy
Start-Transcript -Path $env:TEMP\winget-install$date.log
...
function install-wingetapplication {
    ...
    $arguments = "install --id $appid --silent --force --accept-package-agreements --accept-source-agreements"
    start-process $wingetexe $arguments -WindowStyle Hidden
  
}


function install-appscheduledtask {
    [cmdletbinding()]
...
#Set the action
$action = New-ScheduledTaskAction -Execute "winget.exe" -Argument "upgrade --id $appid --silent --force --accept-package-agreements --accept-source-agreements"

#Set a trigger
$trigger = New-ScheduledTaskTrigger -AtLogOn

#Set a Name
$taskname = "Winget Application Update for $appname"

#Set a Description
$taskdescription = "Checks for an updates $appname on logon via Winget"
...
find-wingetpackage '""' | out-gridview -PassThru -Title "Available Applications" | ForEach-Object {
    ...
    write-host "Installing $appname Application"
    install-wingetapplication -appid $appid -appname $appname
    write-host "$appname Installed"

    ##Install the Scheduled Task
    write-host "Installing Scheduled task for auto-updates of $appname"
    install-appscheduledtask -appid $appid -appname $appname
