...
.Searches all installed Winget apps and displays GridView output
.Removes Application
.Removes Scheduled Task to update application
...
Start-Transcript -Path $env:TEMP\winget-uninstall$date.log
...
function uninstall-wingetapplication {
    ...
    $arguments = "uninstall --id $appid --silent --accept-source-agreements"
    start-process $wingetexe $arguments -WindowStyle Hidden
}


function uninstall-appscheduledtask {
    [cmdletbinding()]
...
get-wingetpackage '""' | out-gridview -PassThru -Title "Available Applications" | ForEach-Object {
    $appid = $_.Id
    $appname = $_.Name
    write-host "$appname Selected with ID of $appid"

    ##Remove the Application
    write-host "Uninstalling $appname Application"
    uninstall-wingetapplication -appid $appid -appname $appname
    write-host "$appname Uninstalled"

    ##Remove the Scheduled Task
    write-host "Uninstalling Scheduled task for auto-updates of $appname"
    uninstall-appscheduledtask -appid $appid -appname $appname
